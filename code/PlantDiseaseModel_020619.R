# error when run on 2/6/19 (now fixed): perennial survival facilitation a function of Ni instead of Nh
...
## Repeat simulation 6 without disease

# repeat simulation over perennial parameters
out8=list()
for(j in 1:length(alpha.pp3)){
  out8[[j]]=compFun(alpha.aa=alpha.aa,alpha.pp=alpha.pp3[j],h.s.comp=F,vir.a=1,vir.p=1)
}

# dataframe
outdf8=data.frame(rbindlist(out8))
str(outdf8)
outdf8$scenario=factor(outdf8$scenario,levels=c("Pre-invasion","Invasion"))

# final day dataframe
findf8=subset(outdf8,time==simtime.i)%>%spread(.,species,N)%>%rename(.,EvA="Elymus adult",EvS="Elymus seedling",Mv="Microstegium",MvB="Microstegium biomass")
findf8$propEv=with(findf8,(EvA+EvS)/(EvA+EvS+Mv))
findf8$propMv=with(findf8,Mv/(EvA+EvS+Mv))

# plot
ggplot(findf8,aes(x=pp,y=propEv,colour=propEv))+geom_point(size=5,shape=15)+scale_colour_gradientn(colors=c("#92D050","white","#002060"))
...
pexcl=subset(outdf6,round(vir.p,2)==0.61&round(pp,4)==0.2233&scenario=="Invasion"&species!="Microstegium biomass")
pexcl$Nscal=pexcl$N/maxN
max(pexcl$Nscal) #0.075

coex=subset(outdf6,round(vir.p,2)==0.61&round(pp,4)==0.0700&scenario=="Invasion"&species!="Microstegium biomass")
coex$Nscal=coex$N/maxN
max(coex$Nscal) #0.014

# combine results with and without disease
findf6$vir.a=vir.a
findf7$vir.a=virMax
findf8$vir.a=1

findf6b=merge(findf6,findf8,all=T)
findf7b=merge(findf7,findf8,all=T)

plot1=ggplot(aexcl,aes(x=time,y=Nscal,colour=species))+geom_line(size=2)+ylim(0,0.125)+scale_colour_manual(values=c("#002060","#44A5B6","#92D050"),name="Population")+theme(axis.text=element_text(size=axisText,colour="black"),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position="none")+xlab("Years")+ylab("Scaled population size")

plot2=ggplot(coex,aes(x=time,y=Nscal,colour=species))+geom_line(size=2)+ylim(0,0.125)+scale_colour_manual(values=c("#002060","#44A5B6","#92D050"),name="Population")+theme(axis.text=element_text(size=axisText,colour="black"),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position=c(0.4,0.85))+xlab("Years")+ylab("Scaled population size")

plot3=ggplot(pexcl,aes(x=time,y=Nscal,colour=species))+geom_line(size=2)+ylim(0,0.125)+scale_colour_manual(values=c("#002060","#44A5B6","#92D050"),name="Population")+theme(axis.text=element_text(size=axisText,colour="black"),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position="none")+xlab("Years")+ylab("Scaled population size")

plot4=ggplot(findf6,aes(x=pp,y=vir.p,colour=propEv))+geom_point(size=5,shape=15)+scale_colour_gradientn(colors=c("#92D050","white","#002060"),name="Ev\nrelative\nabundance")+theme(axis.text=element_text(size=axisText,colour="black"),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position=c(0.55,0.5),plot.margin=unit(c(-15,5.5,5.5,5.5),"pt"))+xlab("Ev intraspp. comp.")+ylab("Ev disease tolerance")

plot5=ggplot(findf7,aes(x=pp,y=vir.p,colour=propEv))+geom_point(size=5,shape=15)+scale_colour_gradientn(colors=c("#92D050","white","#002060"))+theme(axis.text=element_text(size=axisText,colour="black"),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position="none",plot.margin=unit(c(-15,5.5,5.5,5.5),"pt"))+xlab("Ev intraspp. comp.")+ylab("Ev disease tolerance")

plot6=ggplot(findf8,aes(x=pp,y=vir.p,colour=propEv))+geom_point(size=5,shape=15)+scale_colour_gradientn(colors=c("#92D050","white","#002060"))+theme(axis.text.y=element_text(size=axisText,colour="black"),axis.text.x=element_blank(),axis.title=element_text(size=axisTitle,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=legendText),legend.title=element_text(size=legendTitle),legend.key=element_blank(),legend.position="none",plot.margin=unit(c(5.5,5.5,-15,5.5),"pt"))+xlab("")+ylab(" ")+ylim(0.95,1.05)+scale_y_continuous(breaks=c(1.00),labels=sprintf("%.2f",1))

plot4b=plot_grid(plot6,plot4,nrow=2,rel_heights=c(0.1,1))
plot5b=plot_grid(plot6,plot5,nrow=2,rel_heights=c(0.1,1))

setwd("/Users/AmyKendig/Google Drive/Microstegium Bipolaris/R Output")
pdf("CompetitionOutcomes_PlantDiseaseModel_020619.pdf",width=27,height=5.8)
plot_grid(plot1,plot2,plot3,plot4b,plot5b,nrow=1,labels=c("a","b","c","d","e"),label_size=axisTitle)
dev.off()
