...
pdf("PlantDiseaseModel_011119.pdf")
...
# plot
setwd("/Users/AmyKendig/Google Drive/Microstegium Bipolaris/R Output")
pdf("PlantDiseaseModel_EvSeedlingFac_011119.pdf")
for(k in seq(1,nrow(alphas),by=2)){
...
## Figure with potential effect of seedling facilitation

# create dataset
dat1=subset(outdf,pp==1&ap==1&sa==1&aa==0.1)
dat2=subset(outdf2,pp==1&ap==1&sa==1&aa==0.1)
dat1$Facilitation="constant survival"
dat2$Facilitation="facilitative survival"
dat3=merge(dat1,dat2,all=T)

# figure
setwd("/Users/AmyKendig/Google Drive/Microstegium Bipolaris/R Output")
pdf("PlantDiseaseModel_EvFacilitationExample_01119.pdf",width=6,height=6)
ggplot(subset(dat3,species!="Microstegium biomass"))+geom_line(aes(x=time,y=N,colour=species),size=1)+scale_colour_manual(values=c("#002060","#44A5B6","#92D050"))+facet_grid(Facilitation~scenario)+theme(axis.text=element_text(size=16,colour="black"),axis.title=element_text(size=18,colour="black"),panel.background=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"),panel.border=element_rect(color="black",fill=NA),legend.text=element_text(size=14),legend.title=element_text(size=14),legend.key=element_blank(),strip.background=element_blank(),strip.text=element_text(size=18),legend.position=c(0.25,0.85))+xlab("Years")+ylab("Individuals")
dev.off()