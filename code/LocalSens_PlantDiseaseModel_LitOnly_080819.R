## messed with this before switching to simulations - check before running
...
N0.a.ai = filter(a_inv_none, species == "Microstegium" & time == 100)$N
L0.ai = filter(a_inv_none, species == "Microstegium litter" & time == 100)$N
N0.s.ai = filter(a_inv_none, species == "Elymus seedling" & time == 100)$N
N0.p.ai = filter(a_inv_none, species == "Elymus adult" & time == 100)$N

inf_low_tol=simFun(params=c(params, 1, 1),invader="A",N0.a=N0.a.ai,N0.s=N0.s.ai,N0.p=N0.p.ai,L0=L0.ai,simtime=years)
...
inf_low_tol_both_2 <- inf_low_tol_both %>%
	...
	mutate(species = recode(species, "Ev" = "Native", "Microstegium" = "Invader", "Microstegium litter" = "Invader litter"),
	time = time + 300,
	scenario = "both infected")

# combined figure

setwd("/Users/AmyKendig/Google Drive/Microstegium Bipolaris/R Output")
pdf("FullSimulation_PlantDiseaseModel_080819.pdf",width=9,height=5)
p_alone_none_2 %>%
	filter(species == "Native" & time <= 200) %>%
	full_join(filter(a_inv_none_2, species %in% c("Native", "Invader") & time <= 500)) %>%
	full_join(filter(inf_low_tol_both_2, species %in% c("Native", "Invader") & time <= 500))%>%
	ggplot(aes(x = time, y = log(N+1), color = species)) +
	geom_line()+
	scale_colour_manual(values=c("black", "#407879"), name = "Species") +
	theme_bw() +
...
p_alone_none_2 %>%
	filter(species == "Native" & time <= 200) %>%
	full_join(filter(a_inv_none_2, species %in% c("Native", "Invader") & time <= 300)) %>%
		full_join(filter(inf_low_tol_2, species %in% c("Native", "Invader") & time <= 500))%>%
	ggplot(aes(x = time, y = log(N+1), color = species)) +
	geom_line()+
	scale_colour_manual(values=c("black", "#407879"), name = "Species") +
	theme_bw() +
	...
	xlab("Years") +
	ylab("ln(Population size)")
# native population is crashing not from infection, but from stability of invader populations
# if invader tolerance is increased to allow it to cycle again, native can persist even with very low tolerance to disease
